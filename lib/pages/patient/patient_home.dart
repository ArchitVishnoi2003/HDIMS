...
class _PatientHomeState extends State<PatientHome> {
  ...
  CollectionReference get _allergiesRef => _userDoc.collection('allergies');
  ...
  Future<void> _init() async {
    ...
    _allergySub = _allergiesRef.snapshots().listen((snap) async {
      final results = await Future.wait(snap.docs.map((d) async {
        final raw = d.data() as Map<String, dynamic>;
        // Keep raw values to show encrypted badge
        final rawAllergen = raw['allergen'] as String? ?? '';
        final rawSeverity = raw['severity'] as String? ?? '';
        final dec = _privacyEnabled
            ? await EncryptionService.decryptMap(uid, raw)
            : raw;
        return {
          ...dec,
          '_rawAllergen': rawAllergen,
          '_rawSeverity': rawSeverity,
        };
      }));
      if (mounted) setState(() => _allergies = results);
    });

    _appointmentSub = _appointmentsRef
      ...
      if (mounted) setState(() => _appointments = results);
    });
  }

  /// True if the raw value starts with 'enc:'
  bool _isEnc(String? raw) => raw != null && raw.startsWith('enc:');
                ...
                Container(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      ..._allergies.map((d) {
                        final allergen = d['allergen'] as String? ?? '';
                        final severity = d['severity'] as String? ?? '';
                        final rawAllergen = d['_rawAllergen'] as String? ?? '';
                        final isEncrypted = _isEnc(rawAllergen);
                        return Padding(
                              ...
                              if (isEncrypted) ...[
                                ...
                                Padding(
                                  ...
                                  child: Row(
                                    children: [
                                      Icon(Icons.lock,
                                          size: 10, color: Colors.red[300]),
                                      const SizedBox(width: 4),
                                      Expanded(
                                        child: Text(
                                          rawAllergen.length > 40
                                              ? '${rawAllergen.substring(0, 40)}...'
                                              : rawAllergen,
                                          style: TextStyle(
                                              fontSize: 9,
                                              color: Colors.red[300],
                      ...
                      Text(
                        _privacyEnabled
                            ? 'Your allergy data is end-to-end encrypted'
              ...
              if (widget.linkedPatientId != null)
                StreamBuilder<DocumentSnapshot>(
                  ...
                  builder: (_, drSnap) {
                    ...
                    final dr =
                        drSnap.data!.data() as Map<String, dynamic>;
                    return Container(
                      margin: const EdgeInsets.symmetric(horizontal: 20),
                      padding: const EdgeInsets.all(20),
                      decoration: BoxDecoration(
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          if ((dr['blood'] as String? ?? '').isNotEmpty)
                            _drRow(Icons.bloodtype, 'Blood Group',
                                dr['blood']),
                          if ((dr['allergies'] as String? ?? '').isNotEmpty)
                            _drRow(Icons.warning_amber,
                                'Allergies (Doctor)', dr['allergies']),
                          if ((dr['current medication'] as String? ?? '')
                              .isNotEmpty)
                            _drRow(Icons.medication,
                                'Current Medications',
                                dr['current medication']),
                          const SizedBox(height: 8),
                          Text(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    GridView.count(
                      ...
                      children: [
                        _askAICard(context),
                        _quickLinkCard('Medical History', Icons.history,
                            const Color(0xFF4CAF50)),
