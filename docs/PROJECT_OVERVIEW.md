...
| User Type | Role |
|---|---|
| **Hospital / Doctor** | Adds and manages patient profiles; views medical history; requests access to encrypted records |
| **Patient** | Views records added by their doctor; self-records medications, allergies, checkups, appointments, and daily routines; controls whether their data is encrypted |
...
3. View, update, or remove patient records at any time.
...
5. Receive real-time notifications when a doctor requests access to encrypted records; approve or deny with one tap.
6. Use the built-in **AI Health Assistant** (powered by Google Gemini) for diet planning and health Q&A.
